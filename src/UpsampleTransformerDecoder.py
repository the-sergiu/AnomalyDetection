...
class UpsampleTransformerDecoder(nn.Module):
    def __init__(
        ...
        assert (
            input_channels % (2**num_upsamples) == 0
        ), "input_channels must be divisible by 2^num_upsamples"
        ...
        for _ in range(num_upsamples):
            ...
            self.up_blocks.append(
                nn.Sequential(
                    ...
                    ),  # as putea pune Nearest neigh in loc de bilinear
            ...
            for _ in range(num_blocks):
                transformer_blocks.append(
                    TransformerDecoderBlock(input_channels, num_heads, ff_dim)
                )
    ...
    def forward(self, x):
        for up_block, transformer_block_list in zip(
            self.up_blocks, self.transformer_blocks
        ):
